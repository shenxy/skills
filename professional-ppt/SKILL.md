...
如果用户没有提供 PPT 的内容，请主动提问澄清，不要自己编造内容。

### 第2步: 规划 PPT 大纲

请根据用户提供的主题和内容规划 PPT 的大纲。

除非用户明确说明，本技能所生成的 PPT 的受众是非专业人士（如客户等），页数在 10 页左右。

每页的内容要完整、清晰，以便后续步骤能生成高质量的 PPT 页面。除非必要，请始终使用中文。

除非内容只有 1 页，否则都要在第 1 页增加标题页，包含 PPT 的标题、日期等信息；在最后一页增加致谢页，感谢观众的聆听。

### 第3步: 选择风格

除非用户特别指定风格，所有页面都使用以下风格：
1. 总体风格可以参考 McKinsey/BCG 的风格，布局清晰、专业、简洁，不需要背景图片，不需要周边装饰元素，不需要页码、不需要公司名称或 logo;
2. 使用白色背景、深色文本、灰蓝色点缀的配色方案;
3. 选择简洁现代的字体（如Arial、微软雅黑）;

除非用户特别指定风格，所有内容页面（除了标题页和致谢页）都使用以下风格：
1. 顶部标题行使用较大字号（如 36pt）并且加粗，正文使用较小字号（如 24-28pt）;
2. 顶部标题行文字前添加粗蓝色竖条 "▌" 作为视觉标识;
3. 页面内容要详实，多使用图表、图片等视觉元素来增强表达效果;
...
3. 使用 ./template.jpg 作为参考图像（reference）;
4. 生成 2K 分辨率、16:9 比例的 PPT 页面图像。
