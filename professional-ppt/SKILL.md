...
每页的内容要清晰、详尽，以便后续步骤能生成高质量的 PPT 页面。除必要的术语外，内容请始终使用中文。
...
如果可用，使用 subagents 来并行生成 PPT 页面图，最多 3 个并行。

### 第5步: 质量检查（强制执行）

检查生成的每个 PPT 页面图，确保内容清晰可读且设计符合要求，特别要避免下面的情况：
1. 空白页面，或者页面内容与预期不符;
2. 文字太小，或者模糊不清，难以阅读;
3. 版面文字过多，导致页面显得拥挤、杂乱，缺乏层次感;
4. 设计风格不符合要求，比如使用了背景图片、装饰元素，或者配色不正确;
